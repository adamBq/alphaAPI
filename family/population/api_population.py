...
def get_suburb_population(suburb_name, bucket_name=S3_BUCKET_NAME, csv_key=S3_CSV_KEY, codes_key=S3_CODES_KEY):
  """
  ...
  try:
    ...
    if "SUBURB_CODES_MAP" not in code_namespace:
      return json.dumps({"error": "No SUBURB_CODES_MAP found in suburb_codes.py"})

    suburb_codes_map = code_namespace["SUBURB_CODES_MAP"]

  except Exception as e:
    return json.dumps({"error": f"Failed to read/execute suburb_codes.py from S3: {e}"})

  name_to_code = {v: k for k, v in suburb_codes_map.items()}

  suburb_code = name_to_code.get(suburb_name)
  if not suburb_code:
    return json.dumps({"error": f"Suburb code not found for '{suburb_name}'."})
  ...
  if filtered_df.empty:
    return json.dumps({"error": f"No data found for suburb with code '{suburb_code}'."})
