...
def test_get_family_data_success(s3_setup, set_env_vars):
  ...
  for key, value in expected.items():
    assert result[key] == value, f"Expected {key} to be {value}, but got {result.get(key)}"
...
def test_get_family_data_missing_codes_file(s3_setup, set_env_vars, monkeypatch):
  """
