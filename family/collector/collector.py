...
def lambda_handler(event, context):
  ...
  try:
    ...
    zip_link = download_soup.find("a", href=lambda href: href and href.endswith(".zip"))
    if not zip_link:
    ...
    s3_client.put_object(Bucket=S3_BUCKET, Key=S3_KEY, Body=zip_response.content)
    ...
    return {
      "statusCode": 500,
      "body": json.dumps({"error": f"Error processing the datapack: {str(e)}"})
    }
